const {
    ...
    TextEntity, MarkerLineEntity, PinEntity, ModelEntity, PolygonEntity, ImageIconEntity, PathEntity,
    MaprayApi,
    GeoMath, GeoPoint,
...
class App extends StandardUIViewer {
    ...
    onMouseDown( point, event )
    {
        if (event.shiftKey) {
            ...
            this._pick(pickResult => {
                    ...
                    else if (pickResult.entity instanceof PathEntity) {
                        pickResult.entity.setColor( [0, 0, 1] );
                    }
    ...
    loadGIS() {
        ...
        */
        ...
        const polygon = new PolygonEntity( this.viewer.scene );
        polygon.setColor([1.0, 0.2, 0.2]);
        polygon.altitude_mode = AltitudeMode.CLAMP;
        ...
        this._path_list = [];
        for ( let i=0; i<2; i++ ) {
            const path = new PathEntity( this.viewer.scene );
            path.setColor([ 1.0, 0.0, 0.0 ]);
            if ( i === 1 ) path.altitude_mode = AltitudeMode.CLAMP;
            ...
            const pin = new PinEntity( this.viewer.scene );
            pin.addMakiIconPin( "car-15", new GeoPoint(ps[0], ps[1], ps[2]));
            pin.addMakiIconPin( "car-15", new GeoPoint(ps[3], ps[4], ps[5]));
            if ( i === 1 ) pin.altitude_mode = AltitudeMode.CLAMP;
            this.addEntity( pin );
        }
